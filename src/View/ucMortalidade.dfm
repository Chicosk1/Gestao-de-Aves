object frmMortalidade: TfrmMortalidade
  Left = 0
  Top = 0
  Caption = 'Lan'#231'amentos de Mortalidade'
  ClientHeight = 441
  ClientWidth = 624
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = 'Segoe UI'
  Font.Style = []
  Position = poScreenCenter
  TextHeight = 15
  object pnlInfoLote: TPanel
    Left = 0
    Top = 0
    Width = 624
    Height = 50
    Align = alTop
    TabOrder = 0
    object lblLoteSelecionado: TLabel
      Left = 1
      Top = 1
      Width = 622
      Height = 48
      Align = alClient
      Alignment = taCenter
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Segoe UI'
      Font.Style = [fsBold]
      ParentFont = False
      ExplicitWidth = 4
      ExplicitHeight = 21
    end
  end
  object pnlInputs: TPanel
    Left = 0
    Top = 50
    Width = 624
    Height = 341
    Align = alClient
    TabOrder = 1
    object lblData: TLabel
      Left = 20
      Top = 20
      Width = 107
      Height = 15
      Caption = 'Data da Mortalidade'
    end
    object Label1: TLabel
      Left = 20
      Top = 80
      Width = 91
      Height = 15
      Caption = 'Qtd. Aves Mortas'
    end
    object lblObs: TLabel
      Left = 20
      Top = 140
      Width = 111
      Height = 15
      Caption = 'Observa'#231#227'o (Motivo)'
    end
    object dtpDataMortalidade: TDateTimePicker
      Left = 20
      Top = 40
      Width = 150
      Height = 23
      Date = 46052.000000000000000000
      Time = 46052.000000000000000000
      TabOrder = 0
    end
    object edtQuantidade: TEdit
      Left = 20
      Top = 100
      Width = 150
      Height = 23
      NumbersOnly = True
      TabOrder = 1
    end
    object memObservacao: TMemo
      Left = 20
      Top = 160
      Width = 300
      Height = 80
      Lines.Strings = (
        '')
      MaxLength = 255
      ScrollBars = ssVertical
      TabOrder = 2
    end
  end
  object pnlBotoes: TPanel
    Left = 0
    Top = 391
    Width = 624
    Height = 50
    Align = alBottom
    TabOrder = 2
    object btnSalvar: TBitBtn
      AlignWithMargins = True
      Left = 538
      Top = 6
      Width = 75
      Height = 33
      Margins.Left = 5
      Margins.Top = 5
      Margins.Right = 10
      Margins.Bottom = 10
      Align = alRight
      Caption = 'Salvar'
      TabOrder = 0
      OnClick = btnSalvarClick
    end
    object btnCancelar: TBitBtn
      AlignWithMargins = True
      Left = 448
      Top = 6
      Width = 75
      Height = 33
      Margins.Left = 5
      Margins.Top = 5
      Margins.Right = 10
      Margins.Bottom = 10
      Align = alRight
      Caption = 'Cancelar'
      ModalResult = 2
      TabOrder = 1
    end
  end
end
